            ...
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item active" aria-current="page">Data Surat Sakit</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="card p-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h4 class="text-primary fw-bold mb-0 d-flex align-items-center">
                            <i class="bx bx-add-to-queue me-4"></i> Tambah Data Surat Sakit
                        </h4>
                    ...
                    <form action="/admin/surat/surat_sakit/create" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="row">
                            <div class="col-md-6">
                                    ...
                                    <select class="form-control select2" name="id_pasien" required>
                                        ...
                                        @foreach ($pasien as $data)
                                            <option value="{{ $data->id }}"
                                                @if ($surat_sakit && $data->id === $surat_sakit->id_pasien) selected @endif>
                                                {{ $data->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="id_dokter" required>
                                        ...
                                        @foreach ($dokter as $data)
                                            <option value="{{ $data->id }}"
                                                @if ($surat_sakit && $data->id === $surat_sakit->id_dokter) selected @endif>
                                                {{ $data->nama }}
                                            </option>
                                        @endforeach
                                    </select>
